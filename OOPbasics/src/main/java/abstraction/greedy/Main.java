...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < input.length; i += 2) {
            String name = input[i];
            long broika = Long.parseLong(input[i + 1]);
            ...
            if (kvoE.equals("")) {
                continue;
                    ...
                    < torba.values().stream().map(Map::values).flatMap(Collection::stream).mapToLong(e -> e).sum()
                    + broika) {
                continue;
            }
            ...
            switch (kvoE) {
                case "Gem":
                    ...
                    } else if (torba.get(kvoE).values().stream().mapToLong(e -> e).sum() + broika > torba.get("Gold")
                ...
                case "Cash":
                    ...
                    } else if (torba.get(kvoE).values().stream().mapToLong(e -> e).sum() + broika > torba.get("Gem")
                            .values().stream().mapToLong(e -> e).sum()) {
                        continue;
                    }
                    break;
